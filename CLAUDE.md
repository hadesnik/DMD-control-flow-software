...
- **MATLAB is installed locally on this MacBook** — Claude can and should run unit tests here before pushing. Use the repo's **no-arg** runner (the repo's `runtests.m` shadows the built-in and takes no arguments; passing `runtests('tests')` errors with "Too many input arguments"). `matlab` is often not on `PATH`; invoke the app binary directly. Mock-backed tests cover most of the codebase, so local pre-flight catches the majority of regressions.
  - **Always pass `-nodisplay`.** `-batch` alone still lets figures render on macOS, so a test run pops MATLAB windows onto Hillel's screen while he is working — he has asked that this not happen. `-nodisplay` suppresses them, and the full suite is green under it (557/557, 2026-07-26) including `runtests.m`'s `DefaultFigureVisible` guard. Figures are still created invisibly, so tests that build and save them still pass.
    - **The two forms are not equivalent — do not substitute bare `-batch`.** With the guard active, bare `-batch` (display attached, figures invisible) errors `test_MockPLM/exportPatternImages_writesFiles` and `test_timing_analysis/testRunSweepEndToEndMock`; both pass in isolation and under `-nodisplay`, so the failure is order-dependent and unexplained. Root cause not chased, because `-nodisplay` is the only sanctioned invocation. If you ever see those two error together, suspect the invocation before the code.
