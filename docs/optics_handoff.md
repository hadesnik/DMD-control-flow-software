...
**The scale constants moved by +15% and +8%.** If any fixture carries the previous µm-per-pixel figures, it is wrong now — and the two axes did **not** move together, so a single scalar correction will not rescue them.

| | rev 3 — 2026-07-30 | **this revision (rev 4)** |
|---|---|---|
...
| **µm/px, dispersion axis** | 2.3935 | **2.5897** |
| Anamorphic factor | 1.1968 | **1.1240** |
| Axial FWHM | 16.3 µm (scaling law) | **32.6 µm (diffraction table; the law now says 24.1)** |
| Depth walk (peak-to-peak) | 32.0 µm | **35.1 µm** |
| Depth walk status | cancelled by the sample tilt stage — see §6 | **SLM-correctable, per target group — see §6** |
| Depth gradient dz/dx | 0.02888 | **0.02929** |
...
**⚠ The scale below assumes 2 lens swaps that have not happened yet:** f7 (Fourier) 500 -> 250 mm; f6 (SLM relay) 150 -> 80 mm [vs the bench of record, USER 2026-08-08]. Until they do, the merged arm does not exist at these numbers — and a calibration fitted against today's unswapped back end would measure **0.9375× the µm-per-pixel figures in §5** (not a calibration error, a different instrument). Confirm the swaps before trusting any expected value here.
...
* **Refit the scale when the merged arm is built.** Both µm-per-pixel
  ...
  moved too (1.1968 → 1.1240). §5 has the
...
* **The SLM enters the control problem.** Target groups get a per-group
  ...
  number (44 mW, §7), separate from the air-breakdown rule.
...
**The sample-plane scale is anisotropic**, 1.1240× — and the
...
| All-ON alignment cap (§7) | **44 mW** | 58 mW |
| Targets/plane at 1 µW/µm² | ~1085 | ~1258 |
...
| 3 | TF grating | Newport 33010FL01-530R | — | 50 × 50 mm | 34.1 × 25.0 mm lit | 1200 g/mm, 1000 nm blaze. **In 42.8°, out 34.5°** — sets the anamorphic factor and the depth tilt |
| 4 | Ra | ACT508-300-B-ML | 300 | 2″ | 38.1 mm | Grating-side relay, first |
| 5 | Rb | ACT508-200-B-ML | 200 | 2″ | 28.7 mm | Grating-side relay, second. **m_x = 0.667×** to the merge point I0 |
...
| Element | Value | Note |
|---|---|---|
| f7 (Fourier) | **250 mm** | bench carries 500 mm — SWAP (ACT508-250-B-ML). Beam 31.2 mm |
| SLM | Meadowlark 1024 x 1024 (HSP1K class) | at the pupil, 12.4 × 7.2 mm footprint on 17.4 × 17.4 mm (72% worst-axis fill) |
...
```
DMD --80-- L1a --480-- L1b --400-- GRATING --300-- Ra --500-- Rb --200-- I0 --250-- f7 --250-- SLM --80-- f6 --80-- I1 -> periscope -> PBS -> tube -> objective
```
...
m_x    = f_Rb  / f_Ra                      = 200 / 300  = 0.6667
m_i0_s = (f6/f7) * (peri2/peri1) * (EFL_obj/f_tube)
       = (80/250) * (200/100) * (20.0/200) = 0.06400

M_gs   = 1 / (m_x * m_i0_s)                                = 23.438   (grating -> sample demag)
groove-axis scale     = pitch * M1 / M_gs                  = 2.3040 µm/px
dispersion-axis scale = that * cos(beta)/cos(alpha)        = 2.5897 µm/px
```
...
all, but it sets the 1.1240× anisotropy **and** the depth tilt in
...
| Resulting field | **1067 × 1199 µm** ellipse at the sample |
...
```
x_disp   = d_disp   * 2.5897     # µm per pixel, DISPERSION axis
...
| Dispersion | 0.239790 | **2.5897** |

Inverse: `dc = (x_disp/2.5897 + y_groove/2.3040)/sqrt(2)`,
`dr = (x_disp/2.5897 - y_groove/2.3040)/sqrt(2)`.

**To place a round spot at the sample**, draw an ellipse on the DMD compressed
by 1.1240× along the `(1, 1)` diagonal. Drawing a circle in pixels
gives a sample spot 1.12× longer along the dispersion axis.
...
| | |
|---|---|
| Surface tilt | 1.678°, entirely along the **dispersion** axis |
| Depth gradient | **0.02929 µm of depth per µm** across the field |
| ... per DMD pixel | 0.07584 µm per pixel along the `(1, 1)` diagonal |
| Total walk across the patch | **35.1 µm** peak-to-peak |
| Axial FWHM | **32.6 µm**, at a 12 µm target |
| SLM remote focus, 80% blaze | **±904 µm = 52× the walk half-range** |
| Walk correction phase cost | 1.26 wrapped waves across the pupil |

So `z_um ≈ x_disp * 0.02929` (sign from calibration), and the correction
...
field curvature contributes 1.31 µm here and is negligible.
...
0.81 px/fringe (< 2 = unwritable), so the ghost is
...
NA, objective NA) grid — 32.6 µm at this build's NA_spectral
0.163 and groove-resolution factor η = 1.00. Earlier
revisions quoted `physics.axial_fwhm_um`, a scaling law tagged `[LOW ±40%]`
which for this build says 24.1 µm; it runs low against the
...
~189× longer (grooves lit by the patch over twice the
...
* **All-ON / uniform patch (the alignment state): the hazard.** Every
  ...
  44 mW at the grating arm whenever the pattern is all-ON or
  near-uniform.**
* **Sparse operating patterns: benign.** One 12 µm target's
  ...
  (~1085 targets/plane) before the LC
  ceiling binds.
...
* The chip Scheimpflug tilt is 7.8° and **its
...
2.3040 / 2.5897 µm per pixel by more than a few percent,
...
um_per_px_disp: 2.5897
anamorphic: 1.1240
axis_rotation_deg: 45
depth_gradient_um_per_um: 0.02929
axial_fwhm_um: 32.6
walk_um: 35.1
remote_focus_um: 904
dwell_exponent: 2
