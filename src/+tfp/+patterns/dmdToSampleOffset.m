function sampleOffsetUm = dmdToSampleOffset(dmdOffsetPx, mapSpec, varargin)
%dmdToSampleOffset DMD pixel offsets -> sample-plane um offsets (anisotropic, 45-degree clocked chip).
%
%   sampleOffsetUm = dmdToSampleOffset(dmdOffsetPx)
%   sampleOffsetUm = dmdToSampleOffset(dmdOffsetPx, model)
%   sampleOffsetUm = dmdToSampleOffset(dmdOffsetPx, config)
%   sampleOffsetUm = dmdToSampleOffset(dmdOffsetPx, linearMatrix)
%   sampleOffsetUm = dmdToSampleOffset(..., 'mapUnits', 'um')
%
%   The forward half of the bring-up coordinate map (docs/dmd_control_handoff.md
%   §5). The chip is mounted clocked 45 degrees about its own normal, so the
%   optical axes are the chip DIAGONALS, and the sample scale is anisotropic
%   (1.4162 um/px along the grating's dispersion axis, 1.1250 um/px along the
%   grooves). This replaces the scalar `pixelsPerUm` used elsewhere in the repo,
%   which is isotropic, axis-unaware, and ~4x too small.
%
%     d_disp   = sign * (dc + dr) / sqrt(2)     % along the dispersion axis
%     d_groove =        (dc - dr) / sqrt(2)     % along the grooves
%     x_disp   = d_disp   * umPerPixelDispersion
%     y_groove = d_groove * umPerPixelGroove
%
%   Inputs:
%     dmdOffsetPx - 1x2 or N x 2 numeric. Each row is a pixel OFFSET from the
%                   patch centre, [dCol dRow], matching the [col row] ordering
%                   used by tfp.patterns.multiSpot and tfp.patterns.singleSpot.
%                   These are offsets, not absolute pixel coordinates: add the
%                   result of the inverse map to your patch centre yourself.
%     mapSpec     - optional. One of:
%                     [] or omitted - design constants from
%                                     tfp.util.opticalModel() (all defaults).
%                     struct        - a model struct from tfp.util.opticalModel,
%                                     or any config struct; it is passed through
%                                     tfp.util.opticalModel for validation. A
%                                     struct carrying a numeric field
%                                     .dmdToSampleLinear is treated as the
%                                     fitted-affine case below.
%                     2x2 or 3x3    - the LINEAR part of a FITTED DMD->sample
%                       numeric        affine, mapping [dCol dRow] -> [x y] um.
%                                     For a 3x3 the translation row/column is
%                                     ignored (these are offsets). Per §9 of the
%                                     handoff, a fitted affine always wins over
%                                     the design constants — the same call site
%                                     therefore works pre- and post-calibration.
%     options     - optional trailing options struct or name/value pairs (see
%                   "Units guard" below):
%                     'mapUnits'         'um' (asserts the supplied map is
%                                        um-valued; trusted, and the sanctioned
%                                        way to silence the plausibility
%                                        warning), 'camera_px' (errors), or
%                                        'unknown' (default behaviour). A
%                                        struct mapSpec may instead carry the
%                                        tag as a .mapUnits / .units field
%                                        beside .dmdToSampleLinear.
%                     'cameraUmPerPixel' um per substage-camera pixel, used by
%                                        the plausibility check below. Defaults
%                                        to config camera.umPerPixel if the
%                                        caller passed a config.
%
...
%        throws tfp:patterns:dmdToSampleOffset:wrongUnits.
...
%        threshold, tfp:patterns:dmdToSampleOffset:suspiciousMapScale is
...
%
%   Output:
%     sampleOffsetUm - same shape as dmdOffsetPx (1x2 or N x 2). Each row is
%                      [x_disp y_groove] in um: column 1 along the grating's
%                      dispersion axis, column 2 along the grooves.
%
...
%   The groove axis carries no separate sign toggle: a fitted affine supersedes
%   the whole design map, signs included, so only the pre-calibration guess
%   needs the knob. When mapSpec is a matrix its signs are already fitted and
...
%
%   Errors:   tfp:patterns:dmdToSampleOffset:<reason>
%             ...:wrongUnits is the units guard; ...:badUnits / ...:badOption
%             are malformed option arguments.
%   Warnings: tfp:patterns:dmdToSampleOffset:suspiciousMapScale
%
%   See also tfp.patterns.sampleToDmdOffset, tfp.util.opticalModel,
%            tfp.patterns.calibratedAffine, tfp.patterns.multiSpot.

if nargin < 2
    mapSpec = [];
end

opts = parseMapOptions_('dmdToSampleOffset', varargin);
dmdOffsetPx = validateOffsets_(dmdOffsetPx, 'dmdToSampleOffset', 'dmdOffsetPx');
M = resolveLinearMap_(mapSpec, 'dmdToSampleOffset', opts);  % [dc dr] px -> [x y] um

% Rows are points, so transpose in and out: M is a column-vector operator.
sampleOffsetUm = (M * dmdOffsetPx')';
end
